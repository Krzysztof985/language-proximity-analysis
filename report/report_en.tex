...
\subsection{Docker Support (\texttt{docker/})}

Containerization for reproducible environment:
\begin{itemize}
    \item \texttt{Dockerfile}: Python 3.11 base image
    \item \texttt{docker-compose.yml}: Service definition with volume mounts
    \item \texttt{docker-init.sh}: Automated build and start script
    \item \texttt{.dockerignore}: Excludes cache and generated files
\end{itemize}
...
\subsubsection{Statistics for Polish Language}
\begin{table}[H]
\centering
\begin{tabular}{lrr}
\toprule
\textbf{Data type} & \textbf{Number of elements} & \textbf{Source} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Statistics of extracted data for Polish language}
\end{table}

\subsection{CBOW Training}

\subsubsection{Test on Simple Text}
Text: \textit{"the quick brown fox jumps over the lazy dog"}
...
\bottomrule
\end{tabular}
\caption{Training parameters on simple text}
\end{table}

\textbf{Example similarities}:
\begin{itemize}
    \item 't' $\leftrightarrow$ 'e' (0.61)
    \item 'o' $\leftrightarrow$ 'd' (0.53)
    \item 'i' $\leftrightarrow$ 'o' (0.44)
\end{itemize}

\subsubsection{Test on Polish Data}
...
\bottomrule
\end{tabular}
\caption{Training parameters on Polish data}
\end{table}

\textbf{Learned character similarities}:
\begin{itemize}
    \item 'i' $\leftrightarrow$ 'o' (0.44)
    \item 'e' $\leftrightarrow$ 'i' (0.38)
    \item 'a' $\leftrightarrow$ 'e' (0.26)
\end{itemize}
...
\subsubsection{Build and Start Container}
\begin{lstlisting}[language=bash]
cd docker
./docker-init.sh
\end{lstlisting}

\subsubsection{Access Container}
\begin{lstlisting}[language=bash]
docker-compose -f docker/docker-compose.yml exec \
    language-proximity /bin/bash
\end{lstlisting}

\subsubsection{Run Training Inside Container}
\begin{lstlisting}[language=bash]
# Inside container
python src/embeding/train_cbow.py
\end{lstlisting}
